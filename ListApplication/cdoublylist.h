...
DoublyList *insertAtFirst(DoublyList *list,int data)
{
  ...
  {
    Node *temp;
    temp=newNode(data,list->tail,list->head);
    list->tail->next=temp;
...
DoublyList *clearNode(DoublyList *list,Node *thead)
{
  if(thead!=NULL)
    {
      if(thead->next==NULL ||thead->next==list->head)
...
DoublyList *append(DoublyList *list,int data)
{
  ...
  else
  {
    Node *ttail=list->tail;
    Node *temp;
    temp=newNode(data,ttail,list->head);
    list->head->previous=temp;
    list->tail=temp;
    ttail->next=temp;
    list->size++;
...
DoublyList *clear(DoublyList *list,int data)
{
  if(!isEmpty(list))
  {
    ...
    while(thead!=list->tail && thead->data!=data)
      thead=thead->next;
   if(thead->data==data)  
...
/*
Functions uses  DEFAULT type of LinkedList
*/
...
DoublyList *generate (DoublyList *list,int n,int range)
{
  int i;
  
  for(i=0;i<n;i++)
    append(list,rand()%range);
  return list;
}
